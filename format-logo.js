...
(function () {
  ...
  function position(el, key) {
    ...
    const w = Math.min(narrow ? 210 : 270, innerWidth - 24);
    const h = narrow ? 42 : 56;
    el.style.width = w + 'px';
    el.style.height = h + 'px';
    el.style.left = Math.round((innerWidth - w) / 2) + 'px';
    el.style.top = (narrow ? 10 : 18) + 'px';
    el.style.right = 'auto';
  ...
  function add() {
    ...
    el.innerHTML = `<div class="art">${cfg.icon(cfg.colors)}</div><div class="copy"><div class="name">${cfg.name}</div></div>`;
